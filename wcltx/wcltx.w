...
@ An example of .  This example, based on a program by
Klaus Guntermann and Joachim Schrod~\cite{litprog:schrod:cweb}
presents the ``word count'' program from , rewritten in
 to demonstrate literate programming in .%
    \footnote{%
	Incidentally, Klaus Guntermann and Joachim Schrod are now
using this  and have stopped the support for their version.
Joachim Schrod contributed the  style used for the markup of
this document.
    }
...
The purpose of  is to count lines, words, and/or characters in a
...
Most programs share a common structure.  It's probably a
...
defined by this program \texttt{wcltx.w}:
...
do this initialization explicitly; however, 's globals are automatically
...
@ The present section, which does the counting that is 's \emph{raison
...
@ We might as well improve a bit on s by displaying the
number of files too.
...
@ Incidentally, a test of this program against the system
command on a showed that the ``official'' was slightly
slower. Furthermore, although that gave an appropriate error
